...
(() => {
  ...
  function showAnalyticsLoading(clearingData = false) {
      ...
      if (skeletonEl) {
        ...
        if (clearingData) {
          ...
          if (!existingText) {
            ...
            clearingText.textContent = window.GPTPF_MESSAGES.getMessage('ANALYTICS', 'CLEARING_DATA');
    ...
    breakdownElements.forEach(id => {
      ...
      if (el) {
        ...
        if (clearingData) {
          ...
          el.innerHTML = `<div class="clearing-text">${window.GPTPF_MESSAGES.getMessage('ANALYTICS', 'CLEARING_DATA')}</div>`;
        }
      }
    });
    ...
    if (sparklineContent) {
      ...
      if (clearingData) {
        // Create global clearing overlay
        ...
        clearingOverlay.innerHTML = `
          ...
          ${window.GPTPF_MESSAGES.getMessage('ANALYTICS', 'CLEARING_DATA')}
        `;
    ...
    try {
      ...
      if (points.length === 0 || totalFiles === 0) {
        el.innerHTML = `<svg viewBox="0 0 ${w} ${h}" class="sparkline-empty">
          ...
          <text x="${w/2}" y="${h/2}" text-anchor="middle" class="sparkline-no-data" font-size="10" dy="0.35em">${window.GPTPF_MESSAGES.getMessage('ANALYTICS', 'NO_DATA')}</text>
        </svg>`;
        return;
      }
      ...
      if (currentChartType === 'bar') {
        ...
        window.showBarTooltip = (event, value, day) => {
          ...
          const dayLabel = day === 1 ? window.GPTPF_MESSAGES.getMessage('ANALYTICS', 'TODAY') :
                           day === 2 ? window.GPTPF_MESSAGES.getMessage('ANALYTICS', 'YESTERDAY') :
                           window.GPTPF_MESSAGES.getMessage('ANALYTICS', 'DAYS_AGO').replace('$DAYS$', day);
          tooltip.innerHTML = `
            <div class="tooltip-content">
              <div class="tooltip-value">${value}</div>
              <div class="tooltip-label">${window.GPTPF_MESSAGES.getMessage('ANALYTICS', 'FILES_LABEL')}</div>
      ...
      if (label) label.textContent = window.GPTPF_MESSAGES.getMessage('ANALYTICS', 'CHART_ERROR');
      el.innerHTML = `<div class="sparkline-error">${window.GPTPF_MESSAGES.getMessage('ANALYTICS', 'CHART_UNAVAILABLE')}</div>`;
    }
  }
    ...
    chrome.storage.local.get(['telemetryEnabled'], (result) => {
      if (!result.telemetryEnabled) {
        ...
        if (platformContainer) {
          platformContainer.innerHTML = `<div class="breakdown-item"><span class="item-name">${window.GPTPF_MESSAGES.getMessage('ANALYTICS', 'NO_DATA_YET')}</span><span class="item-count">—</span></div>`;
        }
        if (formatContainer) {
          formatContainer.innerHTML = `<div class="breakdown-item"><span class="item-name">${window.GPTPF_MESSAGES.getMessage('ANALYTICS', 'NO_DATA_YET')}</span><span class="item-count">—</span></div>`;
        }
        return;
      }
  ...
  function applyModelTheming() {
    ...
    chrome.tabs.query({active: true, currentWindow: true}, (tabs) => {
      ...
      let currentPlatform = 'chatgpt'; // default
  ...
  function initPeriodSelector() {
    ...
    timeSelect.addEventListener('change', (e) => {
      ...
      if (typeof window.flash === 'function') {
        window.flash(window.GPTPF_MESSAGES.getMessage('ANALYTICS', 'ANALYTICS_UPDATED').replace('$PERIOD$', selectedLabel), 'info');
      }
